...
export class HashSha512_256 extends SimpleEndpoint {
  schema = {
    tags: ["Hashing"],
  ...
  async handle(c: AppContext) {
    ...
    // Compute SHA-512 and truncate to 256 bits (32 bytes)
    const hashBuffer = await crypto.subtle.digest("SHA-512", inputBytes);
    const hashArray = new Uint8Array(hashBuffer).slice(0, 32); // Truncate to 256 bits
